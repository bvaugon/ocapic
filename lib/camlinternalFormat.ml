...
let format_int fmt n =
  let len = String.length fmt in
  let conv plus =
    match fmt.[pred len] with
      | 'd' | 'i' | 'n' | 'l' | 'L' | 'N' ->
          if plus && n >= 0 then "+" ^ string_of_int n
      ...
      | c ->
                  ...
                  ((fun k ->
                      if k = min_int then '4'
                        ...
                        char_of_int (int_of_char '0' +
                                       (18 - (-k mod 10)) mod 10)
                      else char_of_int (int_of_char '0' + (k mod 10))),
                   (fun k ->
                      if k = min_int then 1638
                      else if k < 0 then 3276 + (k - 1) / 10
                      else k / 10))
              | 'o' ->
                  ((fun k -> char_of_int (int_of_char '0' + (k land 0b111))),
                   (fun k -> k lsr 3))
          ...
          let rec f i n =
            if n <> 0 then (
              ...
              Bytes.unsafe_to_string (Bytes.sub buf (succ i) (10 - i))
          in
            f 10 n
  in
  let rec f i minus plus zero =
    match fmt.[i] with
      | '-' -> f (succ i) true plus zero
      | '+' -> f (succ i) minus true zero
      | '0' -> f (succ i) minus plus true
      | ' ' | '#' -> f (succ i) minus plus zero
      | '1' .. '9' ->
          let nstr = conv plus in
          let fsz = int_of_string (String.sub fmt i (len - i - 1)) in
...
let convert_int32 iconv n = Int32.format (format_of_iconvl iconv) n
(*let convert_nativeint iconv n = format_nativeint (format_of_iconvn iconv) n*)
let convert_int64 iconv n = Int64.format (format_of_iconvL iconv) n
...
*)
    failwith "hexadecimal format unsupported"
  | _ ->
...
fun k o acc fmt -> match fmt with
  ...
  | Nativeint (iconv, pad, prec, rest) ->
    ...
    ignore (iconv, pad, prec, rest);
    failwith "nativeint not implemented"
  | Int64 (iconv, pad, prec, rest) ->
